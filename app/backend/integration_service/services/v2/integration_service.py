...
class IntegrationServiceV2:
    ...
    Mejoras V2:
    - Correcta propagación del user_external_id a los eventos importados.
    - Correcta propagación del calendar_title a los eventos importados.
    """
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                for teamup_event in events:
                    try:
                        ...
                        # Crear evento en EventService (Usando V2 para asegurar tipos correctos)
                        event_response = await client.post(
                            f"{self.event_service_url}/v2/events",
                            json=event_payload
                        )
