import {
  Calendar_Repository_Interface,
  Delete_Recursive_Result,
} from "../../domain/repositories/calendar_repository_interface";
...
export class Http_Calendar_Repository implements Calendar_Repository_Interface {
  ...
  async get_all(user_id: string): Promise<Calendar_Model[]> {
    ...
    const my_calendars = all_calendars.filter(
      (cal) => cal.owner_id === user_id
    );
    const other_public_calendars = all_calendars.filter(
      (cal) => cal.owner_id !== user_id && cal.is_public
    );
