...
const CalendarGrid: React.FC<{
      ...
      >
        {months_labels.map((m_label, idx) => (
          ...
          >
            ...
            <div
              className="grid grid-cols-7 gap-1 text-[0.6rem]"
              aria-hidden="true"
            >
              {Array.from({ length: get_days_in_month(year, idx) }).map(
                (_, d_idx) => (
                  <div
                    key={d_idx}
                    className="text-center bg-gray-100 rounded-sm"
                  >
                    {d_idx + 1}
                  </div>
                )
        ...
        if (day_index >= 1 && day_index <= days_in_month) {
          ...
          events.forEach((event) => {
            if (event_occurs_on_day(event, current_day)) {
              const existing = events_in_week.find(
        ...
        if (day_index >= 1 && day_index <= days_in_month) {
          ...
          events.forEach((event) => {
            if (event_occurs_on_day(event, current_day)) {
              ...
              const is_event_start =
                current_day_normalized.getTime() === event_start.getTime();
              ...
              if (is_event_start || is_row_start) {
                ...
                for (let future_col = col + 1; future_col < 7; future_col++) {
                  ...
                  if (
                    future_day_index >= 1 &&
                    future_day_index <= days_in_month
                  ) {
                ...
                const is_end =
                  get_date_only(
                    new Date(year, month, day_index + span_days - 1)
                  ).getTime() >= event_end.getTime();
      ...
      for (let col = 0; col < 7; col++) {
        ...
        if (day_index < 1 || day_index > days_in_month) {
          ...
          day_cells.push(
            <div
              key={`empty-${cell}`}
              className="hidden md:block invisible min-h-[100px] md:min-h-[120px]"
            ></div>
          );
        } else {
          ...
          const day_name =
            days_labels[
              current_day_date.getDay() === 0
                ? 6
                : current_day_date.getDay() - 1
            ];
          const day_events = events.filter((e: Event_Model) =>
            event_occurs_on_day(e, current_day_date)
          );
          ...
          const is_today =
            current_day_date.getDate() === today.getDate() &&
          ...
          day_cells.push(
            <div
              ...
              aria-label={`${
                is_today ? "Hoy, " : ""
              }${day_name} ${day_index}, ${day_events_count} evento${
                day_events_count !== 1 ? "s" : ""
              }`}
            >
              ...
              <div className="md:hidden mt-8 flex flex-col gap-1 px-1">
                {day_events.map((event: Event_Model) => {
                  ...
                  const is_start =
                    current_normalized.getTime() === event_start.getTime();
                  const is_end =
                    current_normalized.getTime() === event_end.getTime();
            ...
            {events_to_render
              ...
              .map((segment, idx) => {
                ...
                for (let col = 0; col < 7; col++) {
                  ...
                  if (day_index >= 1 && day_index <= days_in_month) {
                    const current_day = new Date(year, month, day_index);
                    const event_start = get_date_only(
                      new Date(event.start_time)
                    );
                    const is_event_start =
                      get_date_only(current_day).getTime() ===
                      event_start.getTime();
                    const is_row_start = col === 0;

                    if (
                      event_occurs_on_day(event, current_day) &&
                      (is_event_start || is_row_start)
                    ) {
                      start_col = col;
                      break;
                    }
                ...
                const width_percent =
                  segment.span_days * col_width - gap_adjustment * 2;
                  ...
                  >
                    <div
                      className={`
                                            ...
                                            ${
                                              segment.is_start
                                                ? "pl-2 rounded-l border-l-2"
                                                : "pl-1"
                                            }
                                            ${
                                              segment.is_end
                                                ? "pr-6 rounded-r border-r-2"
                                                : "pr-1"
                                            }
                                            border-t-2 border-b-2 border-basmati-black
                                        `}
                      style={{
                        backgroundColor: event_color,
                        borderLeftColor: segment.is_start
                          ? "#1A1A1A"
                          : "transparent",
                        borderRightColor: segment.is_end
                          ? "#1A1A1A"
                          : "transparent",
                      }}
                    >
      ...
      <section className="w-full" aria-label="Vista mensual del calendario">
        <div className="hidden md:grid grid-cols-7 gap-4 mb-4" role="row">
          {days_labels.map((day) => (
            <div
              key={day}
              className="text-center font-black text-xl uppercase"
              role="columnheader"
            >
              {day}
            </div>
  ...
  if (view === "week") {
    ...
    all_day_in_week.sort(
      (a, b) => b.end_col - b.start_col - (a.end_col - a.start_col)
    );
    ...
    const max_all_day_slots =
      Math.max(0, ...Array.from(all_day_slots.values())) + 1;
    const all_day_section_height =
      all_day_in_week.length > 0 ? max_all_day_slots * 24 + 8 : 0;
        ...
        {all_day_in_week.length > 0 && (
          <div className="flex border-b-3 border-basmati-black bg-gray-50">
            ...
            <div
              ...
              style={{ minHeight: `${all_day_section_height}px` }}
            >
              ...
              {all_day_in_week.map(({ event, start_col, end_col }) => {
                ...
                const event_end = get_date_only(new Date(event.end_time));
                const is_start =
                  get_date_only(week_dates[start_col]).getTime() ===
                  event_start.getTime();
                const is_end =
                  get_date_only(week_dates[end_col]).getTime() ===
                  event_end.getTime();
        ...
        >
          ...
          <div className="flex-1 grid grid-cols-7 relative">
            ...
            {week_dates.map((date, col_idx) => {
              ...
              const day_timed_events = timed_events.filter((e: Event_Model) =>
                event_occurs_on_day(e, date)
              );
                    ...
                    return (
                      <div
                        ...
                        title={`${
                          ...
                          hour: "2-digit",
                          minute: "2-digit",
                        })} a ${event_end.toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}`}
                      >
  ...
  if (view === "day") {
    ...
    const day_events = events.filter((e) => {
      ...
      return (
        e_date.getDate() === currentDate.getDate() &&
        e_date.getMonth() === currentDate.getMonth() &&
        e_date.getFullYear() === currentDate.getFullYear()
      );
    });
    ...
    return (
      <section
        ...
        aria-label={`Vista de día, ${day_events.length} evento${
        ...
        <div className="space-y-4">
          ...
          {day_events.length === 0 ? (
            <p className="text-gray-500 italic">
              No hay eventos para este día. Haz click para crear uno.
            </p>
          ) : (
            day_events.map((event) => {
              ...
              return (
                ...
                >
                  <time
                    className="w-20 font-bold text-gray-500"
...
export const Dashboard_Page = () => {
  ...
  const [event_to_delete, set_event_to_delete] = useState<{
    id: string;
    title: string;
  } | null>(null);
  ...
  const get_header_title = () => {
    ...
    if (view === "day")
      return `${current_date.getDate()} de ${
    ...
    <MainLayout>
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl md:text-4xl font-black uppercase mb-1">
              {get_header_title()}
            </h1>
        ...
        <div
          className="grid grid-cols-2 md:flex gap-2 w-full md:w-auto"
          role="group"
        ...
        <>
          ...
          >
            <p className="text-base">
              ...
              <strong>"{event_to_delete?.title}"</strong>?
            </p>
            ...
            {error && (
              <div
                className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded mt-4"
                role="alert"
              >
