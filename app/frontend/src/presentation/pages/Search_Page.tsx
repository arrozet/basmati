    ...
    return (
        <main className="p-4 md:p-8 w-full max-w-6xl mx-auto">
            <Link to="/dashboard" className="inline-block mb-6">
                <Neo_Button variant="secondary" className="flex items-center gap-2 text-sm font-bold px-4 py-2" aria-label="Volver al calendario principal">
            ...
            <header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                <div className="flex items-center gap-3 w-full">
                    <div className="flex items-center justify-center w-12 h-12 bg-basmati-yellow rounded-none border-3 border-basmati-black shadow-hard flex-shrink-0">
                        <FontAwesomeIcon icon={faSearch} className="text-xl text-basmati-black" />
                    </div>
            ...
            <Neo_Card className="mb-12 p-6 bg-white">
                <form onSubmit={handle_submit} aria-label="Formulario de búsqueda global">
                    <div className="flex flex-col md:flex-row gap-4 items-end">
                        ...
                        <Neo_Button type="submit" variant="primary" className="w-full md:w-auto mb-[2px] flex items-center justify-center gap-2">
            ...
            {loading && (
                <div className="text-lg text-center py-12 flex flex-col items-center gap-4" role="status" aria-live="polite">
                    <div className="animate-spin w-8 h-8 border-4 border-basmati-yellow border-t-basmati-black rounded-full"></div>
                    <span className="font-bold text-gray-600">Buscando resultados...</span>
                </div>
            )}
            ...
            {calendars.length > 0 && (
                <section aria-label="Resultados de calendarios" className="mb-12">
                    <h2 className="text-2xl font-bold mb-6 text-basmati-black flex items-center gap-3 pb-2 border-b-3 border-basmati-black w-fit">
                        <FontAwesomeIcon icon={faCalendarAlt} className="text-basmati-blue" />
                        ...
                        {calendars.map((calendar) => (
                            ...
                            >
                                <div className="flex items-center gap-3 mb-3 border-b-2 border-gray-100 pb-3">
                                    ...
                                    <h3 className="text-xl font-bold truncate" title={calendar.title}>{calendar.title}</h3>
                                </div>
                                <div className="flex items-center gap-2 text-sm text-gray-600 mb-4 px-1">
                                    ...
                                    />
                                    <span className="font-medium">{calendar.is_public ? "Público" : "Privado"}</span>
                                </div>
            ...
            {events.length > 0 && (
                <section aria-label="Resultados de eventos" className="mb-12">
                    <h2 className="text-2xl font-bold mb-6 text-basmati-black flex items-center gap-3 pb-2 border-b-3 border-basmati-black w-fit">
                        <FontAwesomeIcon icon={faClipboardList} className="text-basmati-yellow" />
                        ...
                        {events.map((event) => (
                            ...
                            >
                                ...
                                <div className="pl-3 flex justify-between items-start mb-3">
                                    <h3 className="text-xl font-bold leading-tight line-clamp-2">{event.title}</h3>
                                </div>
                                ...
                                {event.description && (
                                    <p className="pl-3 mb-4 text-gray-700 line-clamp-3 flex-grow text-sm">{event.description}</p>
                                )}
            ...
            {!loading && events.length === 0 && calendars.length === 0 && query && (
                <div className="text-center py-16 bg-white border-3 border-basmati-black border-dashed flex flex-col items-center gap-4 rounded-lg">
                    <FontAwesomeIcon icon={faExclamationCircle} className="text-4xl text-gray-300" />
                    <p className="text-xl text-gray-600 font-bold">No se encontraron resultados para "{query}".</p>
             ...
             {!loading && !query && (
                <div className="text-center py-16 bg-white border-3 border-basmati-black border-dashed flex flex-col items-center gap-4 rounded-lg">
                    ...
                    <p className="text-xl text-gray-600 font-bold">Introduce un término para empezar a buscar.</p>
                </div>
            )}
        </main>
    );
