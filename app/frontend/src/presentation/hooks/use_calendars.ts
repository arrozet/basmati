...
import { use_user_context } from "./use_user_context";
...
export const use_calendars = (user_id: string) => {
   ...
   */
  const create_calendar = async (
    calendar: Omit<Calendar_Model, "id">
  ): Promise<Calendar_Model> => {
    try {
      // Incluir el nombre del usuario actual al crear el calendario
      const calendar_with_creator_name: Omit<Calendar_Model, "id"> = {
        ...calendar,
      ...
      const new_calendar = await create_calendar_use_case.execute(
        calendar_with_creator_name
      );
   ...
   */
  const update_calendar = async (
    calendar: Calendar_Model
  ): Promise<Calendar_Model> => {
    try {
      const updated_calendar = await update_calendar_use_case.execute(calendar);
      set_calendars((prev: Calendar_Model[]) =>
        prev.map((c: Calendar_Model) =>
          c.id === updated_calendar.id ? updated_calendar : c
        )
      );
   ...
   * Si recursive es true, elimina también todos los subcalendarios descendientes y sus eventos.
   * Utiliza el endpoint V2 del backend para eliminación recursiva eficiente.
   */
  const delete_calendar = async (
    id: string,
    ...
    try {
      ...
      // Con recursive=true, el backend V2 elimina subcalendarios y eventos automáticamente
      const context_calendars = recursive ? calendars : undefined;

      const result = await delete_calendar_use_case.execute(
        id,
        context_calendars
      );
      ...
      set_calendars((prev: Calendar_Model[]) =>
        prev.filter((c: Calendar_Model) => !result.deleted_ids.includes(c.id))
      );
    } catch (err: any) {
   ...
   */
  const get_calendar_by_id = async (
    id: string
  ): Promise<Calendar_Model | null> => {
    try {
