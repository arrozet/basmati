...
import { faBell, faCheck, faCheckDouble, faTimes, faComment, faCalendar, faClock, faExclamationCircle } from '@fortawesome/free-solid-svg-icons';
import { use_notifications } from '../../hooks/use_notifications';
import { Notification_Model, Notification_Type } from '../../../domain/models/notification_model';
 ...
 */
export const Notification_Bell: React.FC<Notification_Bell_Props> = ({ external_id }) => {
     ...
     */
    const handle_notification_click = async (notification: Notification_Model) => {
                                    ...
                                    <li key={notification.id}>
                                        ...
                                        >
                                            <div className="flex gap-3">
                                                <div className={`flex-shrink-0 w-10 h-10 rounded-full border-2 border-basmati-black flex items-center justify-center ${
                                                    !notification.is_read ? 'bg-basmati-yellow' : 'bg-gray-100'
                                                }`}>
                                                    <FontAwesomeIcon 
                                                        icon={get_notification_icon(notification.type)} 
                                                        className={!notification.is_read ? 'text-basmati-black' : 'text-gray-500'}
                                                    />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className={`text-sm ${!notification.is_read ? 'font-bold' : 'font-medium'} truncate`}>
                                                        {notification.title}
                                                    </p>
